...
# ===== M14 故事架构 - 迭代结果核验（轻量模型） =====

M14_GAP_CHECK_SYSTEM_PROMPT = """你是架构修订质量检查员，负责核对作者反馈是否都被落实。
...
M14_GAP_CHECK_USER_PROMPT_TEMPLATE = """【作者修改意见】
{feedback}
...
# ===== A. RAG 语义召回 =====
RAG_INJECTION = """【语义召回·关键设定保底（含角色生死/金手指/已揭真相/人物恩怨，勿照抄，但**不得与之矛盾**）】
{rag_context}"""
...
# ============================================================
# M19 复核同步：上游改动后，找下游阶段的未覆盖 / 冲突
# ============================================================
M19_REVIEW_SYSTEM_PROMPT = """你是小说创作复核编辑。上游阶段产物被修改后，下游阶段可能产生「未覆盖」与「冲突」，你的职责是把它们找出来交给作者裁决。
...
M19_REVIEW_USER_TEMPLATE = """【下游待复核阶段】
阶段：{target_label}
