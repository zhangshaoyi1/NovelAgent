...
class M5ContextMixin:
    ...
    def _load_prev_summary(self, chapter_num: int) -> str:
        ...
        二次修复（2026-09-11，五灵破 ch181/182 实证）：旧窗口「头 160 + 尾 300」对
        3000+ 字长章的中段完全盲区——ch181 被判矛盾的细节（铁匣/人数/拇指动作）
        全部落在注入窗口之外，writer 只能自创。现改为：≤ 4000 字给全文；
        超长章给「开头 + 中段采样 + 结尾」，保证中段不再是事实盲区，
        【结尾】仍为本章必须接续的权威状态，并附严禁重演的硬约束。
        """
        if chapter_num <= 1:
            return "（第一章，无前情）"
        ...
        if len(body) <= 4000:
            return (
                body
                + "\n\n（以上为上一章全文。本章必须从上一章结尾处无缝续写，"
                "严禁重演上一章已出现的场景/对话。如果上一章已解决的冲突（如灭掉追兵、"
                "抓住刺客），本章不得让同一事件再次原样发生。）"
            )
        head = body[:600].rstrip()
        tail = body[-1200:].rstrip()
        middle = body[600:-1200].strip()
        mid_block = ""
        if middle:
            mid_block = (
                "\n\n【上一章中段 · 事实补充】中段出现的物品、人数、动作与约定同样是"
                "既定事实，本章不得与之冲突（超长已截取）：\n" + middle[:1800].rstrip()
            )
