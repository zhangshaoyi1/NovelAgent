...
- 依赖检查：复用 ``Gateway`` 注册表（不发起网络调用）；仅当用户加
...
class Doctor:
    ...
    def _probe_embed() -> bool:
        ...
        try:
            ...
            vectors = get_embedding_provider(config).embed(["健康检查探针"])
            return bool(vectors)
    ...
    def _check_deps(self, *, ping: bool = False) -> list[CheckItem]:
        try:
            from agent.client.gateway_adapter import create_gateway, _load_config_from_env

            gateway = create_gateway()
            config = _load_config_from_env()
            cards = gateway.registry.available()
            pre = {
                "provider": cards[0].provider if cards else config.provider,
                "model": cards[0].model if cards else config.model,
                "model_utility": config.model_utility,
                "is_local": config.provider == "ollama",
                "available_providers": [c.provider for c in cards],
                "available_models": [c.model for c in cards],
                "has_fallback": bool(config.fallback_providers),
                "gateway": True,
            }
