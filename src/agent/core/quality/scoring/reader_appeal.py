...
- 计数类维度（人设稳定/设定一致/逻辑漏洞）：对文本中每一个独立的硬伤/漏洞分别列举一条 issue，逐项列举、不得合并多条为一条；不得因"情节需要/伏笔/铺垫/人设成长"等理由豁免；凡确凿的设定/人设/因果冲突均计入（移除"明显"限定）。每个 issue 必须附 quote 字段——从原文摘出、≤30 字的定位凭据；给不出原文引用的问题视为臆测，不得列入。value 必须等于 issues 中计入门禁的条数（severity 为 high 或 mid 计入，low 仅上报）。
