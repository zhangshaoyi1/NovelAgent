...
* `core/quality/scan/bad_point_scanner.py`（A1）：双通道采集 `BadPoint`。
  ...
  * **静态通道**（零 LLM）：字数门禁（<1500 high / <3000 low）、灭门回忆计数（>3 报 self\_repeat）、手段复用统计、编辑残留/标题占位。
...
* 代价：静态规则是估算（关键词近似）；LLM 精扫依赖真实 API 连接，无连接时仅静态结果；agentic\_write 写章路径未注入 reuse\_guard（由 agent 内 WriterAgent 间接覆盖 learnings，记为已知边界）。
