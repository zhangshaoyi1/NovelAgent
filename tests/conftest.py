...
def _build_mock_llm(
    ...
    def chat_side_effect(req, **kwargs):
        ...
        if 'tool_call' in first_content and 'finish' in first_content:
            ...
            # 不够下限会触发「过短章节」保护。此处扩写草稿到 ≥3500 字。
            # 2026-09-12：改为「开头池 × 结尾池」组合造句——整段/模板句重复拼接
            # 会被写时 repetition_abnormal（字符集 Jaccard ≥0.85 占比 ≥30%）
            # 确定性门禁打回（该门禁正是拦"复制凑字"注水），mock 草稿的任意
            # 两句字符集相似度必须低于阈值。
            _openers = (
                "他在崖边盘膝坐定，把最后一缕游丝般的灵气引进气海",
                "山道拐角处，一頭青背狼正低伏着身子，喉咙里滚出威胁的低吼",
                "洞壁上的苔藓泛着幽幽的蓝光，照亮了半截断裂的石碑",
                "师父临终前塞给他的那半块玉佩，此刻在他怀里微微发烫",
                "暴雨冲刷过崖壁，冲出一具半埋在红泥里的白骨",
                "藏经阁的守阁人眯起眼睛，把他从头到脚打量了一遍",
                "炉火忽明忽暗，药汁在鼎里翻出细密的泡，苦气呛人",
                "他数着更声守到后半夜，远处忽然传来两短一长的哨响",
                "月光落在断剑上，剑身缺口处映出他半张灰败的脸",
                "山下的集市散了，炊烟贴着屋脊爬进暮色里，慢慢淡了",
            )
            _ends = (
                "，然后收势，把呼吸压回丹田深处。",
                "，他没有退，反而向前踏了半步。",
            ...
            _parts = [chapter_text]
            _i = 0
            while sum(len(p) for p in _parts) < 3500:
                _parts.append(_openers[_i % 10] + _ends[(_i // 10) % 10])
                _i += 1
            draft = "\n\n".join(_parts)
            action = _json.dumps(
                {
