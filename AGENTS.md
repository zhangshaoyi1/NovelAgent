...
→ 修改后统一运行验证：`pytest`（全量口径 = 无参数，含 tests/ + llmagent_tests/，**2388 passed / 9 skipped / 0 failed**（2397 collected），约 991 秒 / 16 分 30 秒；架构红线 **174 项**；干净跑带 `CODEBUDDY_SAFE_DELETE_ENABLED=0`）
...
5. **pytest 口径**：全量 = `pytest` 无参数（`testpaths = tests + llmagent_tests`，当前 **2388 passed / 9 skipped / 0 failed**（2397 collected），约 991 秒 / 16 分 30 秒；架构红线 **174 项**）；判定看 FAILED 行不看退出码；干净跑须 `CODEBUDDY_SAFE_DELETE_ENABLED=0`；基线见 `../.workbuddy/memory/MEMORY.md`（随提交刷新）。**提交关卡见下文第 10 条。**
