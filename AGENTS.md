...
→ 修改后统一运行验证：`pytest`（全量口径 = 无参数，含 tests/ + llmagent_tests/，**2376 passed / 9 skipped / 0 failed**（2385 collected），约 917 秒 / 15 分 17 秒；架构红线 **174 项**；干净跑带 `CODEBUDDY_SAFE_DELETE_ENABLED=0`）
...
5. **pytest 口径**：全量 = `pytest` 无参数（`testpaths = tests + llmagent_tests`，当前 **2376 passed / 9 skipped / 0 failed**（2385 collected），约 917 秒 / 15 分 17 秒；架构红线 **174 项**）；判定看 FAILED 行不看退出码；干净跑须 `CODEBUDDY_SAFE_DELETE_ENABLED=0`；基线见 `../.workbuddy/memory/MEMORY.md`（随提交刷新）。**提交关卡见下文第 10 条。**
   - ⚠ **后台起跑的 pytest，采集发生在「启动那一瞬间」**：启动后新增的测试不会进本次统计（实测差 5 项，一度被当成回归）。判断口径：`collected` 数与 `passed + skipped` 必须相等；不等就先查是不是采集时间早于最近的测试改动（`pytest --collect-only -q` 复测）。
